import { execute30beachgirlsSingleQuote } from "@/lib/pricing/quote-runtime/adapters/30beachgirls";
...
export function create30beachgirlsAdapter(): ScraperAdapter<BeachGirlsDetailRecord> {
  const runtime = resolveAdapterRuntime({
    managerKey: "30beachgirls",
    defaults: {
  ...
  return {
    ...
    async fetchDetail(context) {
      ...
      const imageUrls = keepDominantImagePattern(
        dedupePreserveOrder(
      ...
      await runRuntimeAdapterQuoteCli(
        {
          ...
          executeSingleQuote: execute30beachgirlsSingleQuote,
    ...
    async runSingleQuoteObservation(input) {
      const result = await execute30beachgirlsSingleQuote({
