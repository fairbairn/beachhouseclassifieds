...
function parseFeeLines(html: string): Array<{ name: string; amount: number }> {
  ...
  const feeSectionMatch = html.match(
    /<ul\s+class="pdp-quote-item-toggle-list">([\s\S]*?)<\/ul>/i,
  );
  ...
  const itemPattern =
    /<span\s+class="pdp-quote-item-text">\s*([^<]+?)\s*<\/span>[\s\S]*?<span\s+class="pdp-quote-item-price"\s+data-price="([^"]+)"/gi;
...
function extractQuoteRequestContext(
  detailHtml: string,
  ...
  const propertyName =
    propertyNameMatch?.[1]?.replace(/&amp;/g, "&").trim() || fallbackName;
...
async function fetchQuoteHtml(input: {
  ...
  for (let attempt = 0; attempt < retryDelaysMs.length; attempt += 1) {
    ...
    try {
      ...
      const feesTopline = parsePriceByLabelContains(html, "Fees");
...
async function buildSidecarForListing(input: {
  ...
  const detail = JSON.parse(detailRaw) as PanhandleDetailRecord;

  if (!detail.html_path) {
    throw new Error(
      `Missing html_path on detail record ${detail.external_listing_id}`,
    );
  }

  const detailHtml = await readFile(detail.html_path, "utf8");
  const quoteContext = extractQuoteRequestContext(detailHtml, detail);
  if (!quoteContext) {
    throw new Error(
      `Missing quote request context for listing ${detail.external_listing_id}`,
    );
  }
...
export async function runPanhandle30aQuoteCli(
  ...
  progress?.phase("starting panhandle30a quote sampling");
  progress?.info(
