...
const DEFAULT_FALLBACK_FEE_PCT = 0.16;
const USER_AGENT =
...
async function buildSidecarForListing(input: {
  ...
  const detail = JSON.parse(detailRaw) as ThirtyABeachDetailRecord;
...
export async function runThirtyABeachQuoteCli(
  ...
  progress?.phase("starting 30abeach quote sampling");
  progress?.info(
