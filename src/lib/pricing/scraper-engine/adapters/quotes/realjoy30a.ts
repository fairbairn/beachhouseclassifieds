...
function buildFallbackHandoffUrl(input: {
  ...
  return `${BASE_HOST}/beach-rentals/book-now?propertyID=${input.listingId}&checkin=${checkin}&checkout=${checkout}`;
}
...
async function buildSidecarForListing(input: {
  ...
  const detail = JSON.parse(detailRaw) as RealJoyDetailRecord;
...
export async function runRealjoy30aQuoteCli(
  ...
  progress?.phase("starting realjoy30a quote sampling");
  progress?.info(
  ...
  progress?.success(
    `realjoy30a quote sampling complete listings=${summaries.length}`,
  );
}

export async function runRealjoy30aSingleQuoteObservation(
  input: SingleQuoteObservationInput,
): Promise<SingleQuoteObservationResult> {
  const propertyName =
    input.title || input.listingId || new URL(input.detailUrl).pathname;
