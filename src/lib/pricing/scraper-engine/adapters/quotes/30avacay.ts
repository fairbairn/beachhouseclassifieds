...
async function buildSidecarForListing(input: {
  ...
  const detail = JSON.parse(raw) as ThirtyAVacayDetailRecord;
...
export async function runThirtyAVacayQuoteCli(
  ...
  progress?.phase("starting 30avacay quote sampling");
  progress?.info(
  ...
  if (selected.length === 0) {
    ...
    progress?.success(
      `30avacay quote sampling complete listings_selected=${totalSelected} processed=0 skipped_existing=${skippedExisting}`,
    );
    return;
  }
