...
const ADAPTER_KEY = "fivestar30a" as const;
const BASE_HOST = "https://www.fivestargulfrentals.com";
...
async function buildSidecarForListing(input: {
  ...
  const detail = JSON.parse(raw) as FiveStarDetailRecord;
...
export async function runFiveStar30aQuoteCli(
  ...
  progress?.phase("starting fivestar30a quote sampling");
  progress?.info(
  ...
  if (selected.length === 0) {
    ...
    progress?.success(
      `fivestar30a quote sampling complete listings_selected=${totalSelected} processed=0 skipped_existing=${skippedExisting}`,
    );
    return;
  }
  ...
  progress?.success(
    `fivestar30a quote sampling complete listings_selected=${totalSelected} processed=${summaries.length} skipped_existing=${skippedExisting}`,
  );
}

export async function runFiveStar30aSingleQuoteObservation(
  ...
  const raw = await fetchRouterQuote({
    ...
    locationId: DEFAULT_HANDOFF_LOCATION_ID,
