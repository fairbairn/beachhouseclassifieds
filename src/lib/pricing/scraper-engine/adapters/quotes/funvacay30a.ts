...
function extractRcavIdentity(detail: FunVacay30ADetailRecord): {
  itemEid: string;
  typeId: string;
  inventoryId: string;
} {
  const source = detail.description_expanded ?? "";
  ...
  throw new Error(
    `Missing rcav identity fields for listing ${detail.external_listing_id}`,
  );
}
...
async function buildSidecarForListing(input: {
  ...
  const detail = JSON.parse(raw) as FunVacay30ADetailRecord;
  ...
  const rcavIdentity = extractRcavIdentity(detail);
...
export async function runFunVacay30AQuoteCli(
  ...
  progress?.phase("starting funvacay30a quote sampling");
  progress?.info(
  ...
  if (selected.length === 0) {
    ...
    progress?.success(
      `funvacay30a quote sampling complete listings_selected=${totalSelected} processed=0 skipped_existing=${skippedExisting}`,
    );
    return;
  }
  ...
  progress?.success(
    `funvacay30a quote sampling complete listings_selected=${totalSelected} processed=${summaries.length} skipped_existing=${skippedExisting}`,
  );
}

export async function runFunVacay30ASingleQuoteObservation(
  ...
  const rcavIdentityFromHandoff = (() => {
    ...
    try {
      ...
      // Fall back to legacy identity extraction.
    }

    return null;
  })();

  const detail: FunVacay30ADetailRecord = {
    external_listing_id: input.listingId,
    detail_url: input.detailUrl,
    description_expanded: input.descriptionExpanded ?? undefined,
  };
  const rcavIdentity = rcavIdentityFromHandoff ?? extractRcavIdentity(detail);
