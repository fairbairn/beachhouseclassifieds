...
const ADAPTER_KEY = "beachblue" as const;
const BASE_HOST = "https://www.beachblueproperties.com";
const ROUTER_ENDPOINT = `${BASE_HOST}/vacation-rentals/router/`;
const CHECKOUT_PATH = `${BASE_HOST}/vacation-rentals/checkout/`;
const DEFAULT_LOCATION_ID = 1;
const HANDOFF_PERSONS = 1;
...
function parseTaxesLine(
  ...
  const taxes = value as RouterTaxes;
  const amount = parseMoney(taxes.amount);
  if (amount === null) {
    return [];
  }

  return [{ name: "Taxes", amount }];
}
...
function buildCheckoutUrl(input: {
  ...
  params.set("persons", String(HANDOFF_PERSONS));
...
async function fetchRouterQuote(input: {
  ...
  const nights =
    (toUnixSecondsAtUtcMidnight(input.endDate) -
      toUnixSecondsAtUtcMidnight(input.startDate)) /
    86400;
  ...
  const payload = {
    ...
    unitId: input.unitId,
    people: persons,
    arrive: toUsDate(input.startDate),
    depart: toUsDate(input.endDate),
    nights,
    optIn: false,
  ...
  const defaultUnavailable: RawObservation = {
    ...
    quoteUnavailableReason: "Router response unavailable",
    baseTotal: null,
    taxesTotal: null,
    feesTotal: null,
    grandTotal: null,
  ...
  for (let attempt = 0; attempt <= MAX_RATE_LIMIT_RETRIES; attempt += 1) {
    ...
    const taxesLines = parseTaxesLine(rawPayload.taxes);
    ...
    const feesTotal = sumFeeLines(allFeeLines);
    ...
    const computedGrandTotal =
      baseTotal !== null
        ? roundCurrency(baseTotal + feesTotal + taxesTotal)
        : null;
    const grandTotal = bookingTotal ?? computedGrandTotal;

    if (!isAvailable || baseTotal === null) {
      return {
        ...defaultUnavailable,
        quoteUnavailableReason:
          apiError ?? errorMsg ?? "Dates unavailable for selected stay window",
        baseTotal,
        taxesTotal: taxesTotal > 0 ? taxesTotal : null,
        feesTotal: feesTotal > 0 ? feesTotal : null,
        grandTotal,
...
async function buildSidecarForListing(input: {
  ...
  const detail = JSON.parse(raw) as BeachBlueDetailRecord;
...
export async function runBeachBlueQuoteCli(
  ...
  progress?.phase("starting beachblue quote sampling");
  progress?.info(
  ...
  if (selected.length === 0) {
    ...
    progress?.success(
      `beachblue quote sampling complete listings_selected=${totalSelected} processed=0 skipped_existing=${skippedExisting}`,
    );
    return;
  }
  ...
  progress?.success(
    `beachblue quote sampling complete listings_selected=${totalSelected} processed=${summaries.length} skipped_existing=${skippedExisting}`,
  );
}
