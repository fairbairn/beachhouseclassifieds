import { executeRosemary30ASingleQuote } from "@/lib/pricing/quote-runtime/adapters/rosemary30a";
...
const DEFAULT_ANCHOR_URL = "https://rosemarybeach.com/rentals/?sort_by=random";
...
function toValidDetailUrl(value: string): string | null {
  try {
    ...
    if (!parsed.hostname.endsWith("rosemarybeach.com")) {
      return null;
    }
...
function extractImageUrlsFromListingRow(
  row: Rosemary30AListingRow | null,
): string[] {
  if (!row) {
    return [];
  }
...
async function loadListingRows(
  ...
  if (!pending) {
    pending = (async () => {
      const allRows: Rosemary30AListingRow[] = [];
...
function buildSeoMap(
  rows: Rosemary30AListingRow[],
): Map<string, Rosemary30AListingRow> {
  const out = new Map<string, Rosemary30AListingRow>();
...
function expandRateDays(
  ...
  availabilityByDate: Map<string, Rosemary30ADayCode>,
): Array<{
  date: string;
  nightly_rate: number | null;
  min_nights: number | null;
...
export function createRosemary30AAdapter(): ScraperAdapter<Rosemary30ADetailRecord> {
  const runtime = resolveAdapterRuntime({
    managerKey: "rosemary30a",
    defaults: {
    ...
    aliases: {
      DETAIL_FETCH_DELAY_MS: [
        "ROSEMARY30A_DETAIL_FETCH_DELAY_MS",
        "THIRTYABEACH_DETAIL_FETCH_DELAY_MS",
      ],
      DETAIL_FETCH_CONCURRENCY: [
        "ROSEMARY30A_FETCH_CONCURRENCY",
        "THIRTYABEACH_FETCH_CONCURRENCY",
      ],
      AVAILABILITY_HORIZON_DAYS: [
        "ROSEMARY30A_AVAILABILITY_HORIZON_DAYS",
        "THIRTYABEACH_AVAILABILITY_HORIZON_DAYS",
      ],
      MAX_CALENDAR_ADVANCE_MONTHS: [
        "ROSEMARY30A_CALENDAR_MAX_MONTHS",
        "THIRTYABEACH_CALENDAR_MAX_MONTHS",
      ],
    },
  });
  ...
  return {
      ...
      await runRuntimeAdapterQuoteCli(
        {
          ...
          executeSingleQuote: executeRosemary30ASingleQuote,
    ...
    async runSingleQuoteObservation(input) {
      const result = await executeRosemary30ASingleQuote({
