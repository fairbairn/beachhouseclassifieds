...
import { runGrayt30AQuoteCli } from "./quotes/grayt30a";
...
const DEFAULT_ANCHOR_URL =
  "https://www.grayt30avacations.com/30a-vacation-rentals#q=*%3A*";
const EXPECTED_LISTING_COUNT = 35;
    ...
    page.evaluate(() => {
      ...
      const toNormalized = (hrefValue: string): string => {
        try {
          ...
          if (!absolute.hostname.endsWith("grayt30avacations.com")) {
            return "";
          }
...
async function fetchDetail(
  ...
  try {
    ...
    console.warn(`[grayt30a] detail pull failed for ${detailUrl}: ${message}`);
    return null;
  } finally {
    await page.close();
  }
...
export function createGrayt30AAdapter(): ScraperAdapter<LuxuryDetailRecord> {
  return {
    managerKey: "grayt30a",
    scriptLabel: "grayt30a",
    defaultAnchorUrl: DEFAULT_ANCHOR_URL,
    detailFetchDelayMs: Math.max(
      0,
      Number(process.env.GRAYT30A_DETAIL_FETCH_DELAY_MS ?? "120") || 120,
    ),
    detailFetchConcurrency: Math.max(
      1,
      Number(process.env.GRAYT30A_DETAIL_FETCH_CONCURRENCY ?? "4") || 4,
    ),
    availabilityHorizonDays: Math.max(
      1,
      Number(process.env.GRAYT30A_AVAILABILITY_HORIZON_DAYS ?? "730") || 730,
    ),
    maxCalendarAdvanceMonths: Math.max(
      8,
      Number(process.env.GRAYT30A_CALENDAR_MAX_MONTHS ?? "26") || 26,
    ),
    isValidDetailUrl(value: string): string | null {
      try {
        const parsed = new URL(value.trim());
        if (
          !parsed.hostname.endsWith("grayt30avacations.com") ||
