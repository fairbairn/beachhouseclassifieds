...
import {
  runThirtyAVacayQuoteCli,
  runThirtyAVacaySingleQuoteObservation,
} from "./quotes/30avacay";
...
const DEFAULT_ANCHOR_URL =
  "https://www.30a-vacay.com/vacation-rentals/results/?Type=House";
const EXPECTED_LISTING_COUNT = 70;
    ...
    page.evaluate(() => {
      ...
      const toNormalized = (hrefValue: string): string => {
        try {
          ...
          if (!absolute.hostname.endsWith("30a-vacay.com")) {
            return "";
          }
...
export function createThirtyAVacayAdapter(): ScraperAdapter<LuxuryDetailRecord> {
  return {
    managerKey: "30avacay",
    ...
    async runSingleQuoteObservation(input) {
      return runThirtyAVacaySingleQuoteObservation(input);
    },
  };
}
