...
const DEFAULT_ANCHOR_URL =
  "https://www.sandersbeachrentals.com/30a-vacation-rentals#q=*%3A*";
const EXPECTED_LISTING_COUNT = 73;
...
function isLikelyDetailPath(pathname: string): boolean {
  ...
  if (!normalizedPath.startsWith("/30a-vacation-rentals/")) {
    return false;
  }
...
async function discoverListings(
  ...
  // Sanders often starts above a heavy hero/nav fold; force list context first.
    ...
    page.evaluate(() => {
      ...
      const toNormalized = (hrefValue: string): string => {
        try {
          ...
          if (!absolute.hostname.endsWith("sandersbeachrentals.com")) {
            return "";
          }
      ...
      // Sanders also exposes listing URLs in "find property by name" option values.
      for (const option of Array.from(
        document.querySelectorAll("select option[value]"),
      )) {
  ...
  let stagnantSteps = 0;
  // Sanders relies on long-running incremental lazy load; allow deeper passes.
...
async function fetchDetail(
  ...
  try {
    ...
    const normalizedMatchingProfile = {
      source: "pm_sandersbeach30a" as const,
    ...
    console.warn(
      `[sandersbeach30a] detail pull failed for ${detailUrl}: ${message}`,
    );
    return null;
  } finally {
    await page.close();
...
export function createSandersBeach30AAdapter(): ScraperAdapter<LuxuryDetailRecord> {
  return {
    managerKey: "sandersbeach30a",
    scriptLabel: "sandersbeach30a",
    defaultAnchorUrl: DEFAULT_ANCHOR_URL,
    detailFetchDelayMs: Math.max(
      0,
      Number(process.env.SANDERSBEACH30A_DETAIL_FETCH_DELAY_MS ?? "120") || 120,
    ),
    detailFetchConcurrency: Math.max(
      1,
      Number(process.env.SANDERSBEACH30A_DETAIL_FETCH_CONCURRENCY ?? "4") || 4,
    ),
    availabilityHorizonDays: Math.max(
      1,
      Number(process.env.SANDERSBEACH30A_AVAILABILITY_HORIZON_DAYS ?? "730") ||
        730,
    ),
    maxCalendarAdvanceMonths: Math.max(
      8,
      Number(process.env.SANDERSBEACH30A_CALENDAR_MAX_MONTHS ?? "26") || 26,
    ),
    isValidDetailUrl(value: string): string | null {
      try {
        const parsed = new URL(value.trim());
        if (
          !parsed.hostname.endsWith("sandersbeachrentals.com") ||
