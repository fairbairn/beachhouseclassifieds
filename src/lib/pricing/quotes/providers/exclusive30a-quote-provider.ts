...
function resolveNumericPropertyId(value: unknown): string {
  if (typeof value === "number" && Number.isFinite(value)) {
  ...
  const rawObservations = await runWithConcurrency(
    ...
    async (index) => {
      ...
      return fetchQuote({
        ...
        listingId: resolveNumericPropertyId(detail.quote_context?.property_id),
        startDate,
        endDate,
...
export async function runExclusive30aSingleQuoteObservation(
  input: SingleQuoteObservationInput,
): Promise<SingleQuoteObservationResult> {
  const startedAt = performance.now();
  const propertyId = resolveNumericPropertyId(input.quoteContext?.property_id);
  if (!propertyId) {
    return {
      ...
      observation: {
        ...
        reason: "missing_quote_context_property_id",
      },
    };
  }
