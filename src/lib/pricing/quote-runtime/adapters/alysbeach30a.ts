...
const ADAPTER_KEY = "alysbeach30a" as const;
const BASE_HOST = "https://vacation.alysbeach.com";
const USER_AGENT =
...
function buildCheckAvailabilityUrl(input: {
  ...
  url.searchParams.set("obj[Arrival]", formatUsDate(input.startDate));
  url.searchParams.set("obj[Departure]", formatUsDate(input.endDate));
  ...
  url.searchParams.set("obj[PromoCode]", "");
  url.searchParams.set("obj[Vendor]", "Track");
...
export async function executeAlysBeach30ASingleQuote(
