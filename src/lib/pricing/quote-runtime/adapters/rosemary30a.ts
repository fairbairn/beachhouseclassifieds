...
const DEFAULT_ORIGIN = "https://rosemarybeach.com";
const USER_AGENT =
...
function buildFallbackCheckoutUrl(input: {
  listingId: string;
  checkInIso: string;
  checkOutIso: string;
  adults: number;
  ...
  params.set("oc", String(Math.max(1, input.adults)));
  params.set("os", String(Math.max(0, input.children)));
  return `${DEFAULT_ORIGIN}/checkout/?${params.toString()}`;
}
...
export async function executeRosemary30ASingleQuote(
  ...
  try {
    ...
    const fallbackHandoffUrl = buildFallbackCheckoutUrl({
      listingId: input.listingId,
      checkInIso: input.checkInIso,
      checkOutIso: input.checkOutIso,
