...
const ADAPTER_KEY = "paradise30a" as const;
const BASE_HOST =
...
function extractQuoteContext(
  ...
  const listingId = asOptionalString(context?.listing_id) ?? input.listingId;
  const locid =
    asOptionalString(context?.locid) ?? asOptionalString(context?.loc_id);
  const detailUrl =
    asOptionalString(context?.detail_url) ??
    `${BASE_HOST}/vacation-rentals/${listingId}`;
...
export async function executeParadise30aSingleQuote(
  input: QuoteExecutionRequest,
): Promise<QuoteExecutionResult> {
  const startedAt = performance.now();
  const timeoutMs = normalizeTimeoutMs(input.options?.timeoutMs);

  let quoteContext: Paradise30AQuoteContext;
  try {
