...
function parseTaxesLines(
  value: unknown,
): Array<{ name: string; amount: number }> {
  if (Array.isArray(value)) {
    return parseFeeLines(value);
  }
...
function extractQuoteContext(
  ...
  const detailUrl = asOptionalString(context?.detail_url) ?? `${BASE_HOST}/`;

  const locationId =
    asPositiveInt(context?.location_id) ??
    asPositiveInt(context?.locationId) ??
    DEFAULT_HANDOFF_LOCATION_ID;
...
export async function executeBeachblueSingleQuote(
  input: QuoteExecutionRequest,
): Promise<QuoteExecutionResult> {
  const startedAt = performance.now();
  const timeoutMs = normalizeTimeoutMs(input.options?.timeoutMs);

  let quoteContext: BeachblueQuoteContext;
  try {
  ...
  const handoffUrl = buildCheckoutUrl({
    ...
    locationId: quoteContext.locationId,
    checkInIso: input.checkInIso,
    checkOutIso: input.checkOutIso,
  ...
  try {
    ...
    const taxesLines = parseTaxesLines(rawPayload.taxes);
