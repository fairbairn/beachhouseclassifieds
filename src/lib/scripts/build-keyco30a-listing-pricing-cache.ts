...
async function main(): Promise<void> {
  ...
  const assumptionsStore = readJson<KeycoAssumptionsStore>(assumptionsRaw);

  const avgFeePct = Number(
    assumptionsStore.assumptions?.avg_fee_pct_of_base ?? 0.19,
  );
...
main().catch((error: unknown) => {
  ...
  console.error(`Failed to build keyco30a listing pricing cache: ${message}`);
  process.exit(1);
});
