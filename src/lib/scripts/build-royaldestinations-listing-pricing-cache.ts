...
type RoyalDestinationsQuoteSidecarRecord = {
  observations?: CanonicalQuoteObservation[];
};

type RoyalDestinationsAssumptionsStore = {
  assumptions?: {
...
async function main(): Promise<void> {
  ...
  const assumptionsStore = readJson<RoyalDestinationsAssumptionsStore>(
    assumptionsRaw,
  );

  const avgFeePct = Number(
    assumptionsStore.assumptions?.avg_fee_pct_of_base ?? 0.03,
  );
  const avgTaxPct = Number(
    assumptionsStore.assumptions?.avg_tax_pct_of_base ?? 0.12,
  );
  ...
  const assumptionsAnchorBase =
    median(sampleAnchors) ?? roundCurrency(GLOBAL_DEFAULT_BASE_NIGHTLY * 0.92);
  ...
  for (const fileName of selected) {
    ...
    const detail = readJson<RoyalDestinationsDetailRecord>(raw);
    ...
    try {
      ...
      const quoteSidecar = readJson<RoyalDestinationsQuoteSidecarRecord>(
        quoteRaw,
      );
      quoteAnchorsByDate = readQuoteAnchorsByDate(quoteSidecar.observations);
    } catch {
      // Quote sidecars are optional for this adapter bootstrap phase.
    }
...
main().catch((error: unknown) => {
  ...
  console.error(
    `Failed to build royaldestinations listing pricing cache: ${message}`,
  );
  process.exit(1);
});
