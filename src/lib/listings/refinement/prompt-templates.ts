export const LISTING_REFINEMENT_PROMPT_BASE = [
  ...
  "Sleeping extraction constraints: always produce non-empty sleeping_arrangements when bedrooms and sleeps are provided and source has sleeping details.",
  "Sleeping extraction must reconcile sleeps exactly through sleeping_summary and bunk_configurations.",
  "Do not return empty sleeping_arrangements unless source explicitly has no sleeping details.",
] as const;

export const REBUILD_HELPFUL_HINTS_PROMPT =
...
export function buildListingRefinementPrompt(input: {
  ...
  return [
    ...LISTING_REFINEMENT_PROMPT_BASE,
    input.rebuildHelpfulHints ? REBUILD_HELPFUL_HINTS_PROMPT : "",
  ]
    .filter(Boolean)
    .join(" ");
}

export const LISTING_REFINEMENT_AUDIT_PROMPT_BASE = [
