...
| sandersbeach30a         | streamline      | yes              | no                      | quote_window_avg  | template_url    | seeded      | [ ]     | RCAPI quote path validated (`/rcapi/item/avail/search`) with scrape-time `entity.eid/id/type` extraction from detail Riot payload; next run pricing cache build + handoff alignment to promote `ready` |
