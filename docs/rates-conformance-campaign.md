...
Last generated: 2026-03-28T20:25:00Z
...
|    2 | keyco30a                |   380 |     0 | 380 | in_progress | no               | yes                     | yes              | custom_hybrid   | Next target adapter. Profile and assumptions exist (1 sample). Next: validate direct quote retrieval hints, build listing pricing cache pipeline, then complete handoff signature proof.                                                     |
...
|    4 | 30aescapes              |   169 |     0 | 169 | in_progress | no               | yes                     | yes              | track_bluetent  | Conformity artifacts now seeded: `pricing-profile.json` added, quote/handoff signatures persisted, and first checkout assumptions sample recorded. Next: add 2+ assumptions samples and wire quote parser into normalized UX pricing output. |
...
1. keyco30a (380 files, gap 380) - next active target: validate direct quote path, expand assumptions to 3/3, and add listing pricing cache generation under details.
2. homeownerscollection30a (208 files, gap 208) - `rcItemAvailForm` already exposes `/rescms/ajax/item/pricing/simple`; implement deterministic quote request shaping + nightly normalization.
...
4. 30aescapes (169 files, gap 169) - Quote/handoff signatures are now persisted in pricing profile and first assumptions sample is recorded; finish assumptions sample minimum and pricing-engine output wiring.
...
- Current readiness gap for 30aescapes remains sample-depth + runtime integration (not contract discovery).
